app.controller('manageContactDetailCtrl',function($scope, $http, $window, $filter, $location) {
	...
	$scope.setflag=function(){
		$scope.address1Error=0;
		$scope.address2Error=0;
		$scope.address3Error=0;
		$scope.countrynameError=0;
		$scope.statenameError=0;
		$scope.citynameError=0;
		$scope.companynameError=0;
		$scope.address1workError=0;
		$scope.address2workError=0;
		$scope.address3workError=0;
		$scope.countrynameworkError=0;
		$scope.statenameworkError=0;
		$scope.emailError=0;
		$scope.mobilenumberworkError=0;
		
		
	}


	$scope.editcontactdetailmobile = function(memberid) {
		...
		var businessnature = $scope.businessnature;
		var faxnumber = $scope.faxnumber;
		var aboutbusiness = $scope.aboutbusiness;
		var businesskeywords = $scope.businesskeywords;
		var website = $scope.website;
		var email = $scope.email;
		var address1work = $scope.address1work;
		var address2work = $scope.address2work;
		...
		var idealreferral = $scope.idealreferral;
		var topproduct = $scope.topproduct;
		var topproblemsolved = $scope.topproblemsolved;
		var networks = $scope.networks;
		var interests = $scope.interests;
		var accomplishments = $scope.accomplishments;
		var businessgoals = $scope.businessgoals;
		var mobilenumberwork = $scope.mobilenumberwork;			
		var communication = $scope.communication;		
		...
		if(!address1) {
			$scope.address1Error=1;
			$scope.address1Msg="Please enter address line-1";
			document.getElementById("address1").focus();
			return;
		}else if(!address2) {
			$scope.address2Error=1;
			$scope.address2Msg="Please enter address line-2";
			document.getElementById("address2").focus();
			return;
		}else if(!address3) {
			$scope.address3Error=1;
			$scope.address3Msg="Please enter address line-3";
			document.getElementById("address3").focus();
			return;
		}else if(!countryname) {
			$scope.countrynameError=1;
			$scope.countrynameMsg="Please enter countryname";
			document.getElementById("countryname").focus();
			return;
		}else if(!statename) {
			$scope.statenameError=1;
			$scope.statenameMsg="Please enter statename";
			document.getElementById("statename").focus();
			return;
		}else if(!cityname) {
			$scope.citynameError=1;
			$scope.citynameMsg="Please enter cityname";
			document.getElementById("cityname").focus();
			return;
		}else if(!companyname) {
			$scope.companynameError=1;
			$scope.companynameMsg="Please enter companyname ";
			document.getElementById("companyname").focus();
			return;
		}else if(!address1work) {
			$scope.address1workError=1;
			$scope.address1workMsg="Please enter address1work";
			document.getElementById("address1work").focus();
			return;
		}else if(!address2work) {
			$scope.address2workError=1;
			$scope.address2workMsg="Please enter address2work";
			document.getElementById("address2work").focus();
			return;
		}else if(!address3work) {
			$scope.address3workError=1;
			$scope.address3workMsg="Please enter address3work";
			document.getElementById("address3work").focus();
			return;
		}else if(!countrynamework) {
			$scope.countrynameworkError=1;
			$scope.countrynameworkMsg="Please enter countrynamework";
			document.getElementById("countrynamework").focus();
			return;
		}else if(!statenamework) {
			$scope.statenameworkError=1;
			$scope.statenameworkMsg="Please enter statenamework";
			document.getElementById("statenamework").focus();
			return;
		}else if(!email) {
			$scope.emailError=1;
			...
			document.getElementById("email").focus();
			return;
		}else if(!mobilenumberwork) {
			$scope.mobilenumberworkError=1;
			$scope.mobilenumberworkMsg="Please enter mobilenumber";
			document.getElementById("mobilenumberwork").focus();
			return;
		}else {
			$scope.spin = 1;
			...
			var link = baseUrl+'editContactDetailMobile?memberid='+memberid+'&address1='+address1+'&address2='+address2+'&address3='+address3+'&statename='+statename+'&cityname='+cityname+'&occupation='+occupation+'&designation='+designation+'&companyname='+companyname+'&businessnature='+businessnature+'&faxnumber='+faxnumber+'&website='+website+'&aboutbusiness='+$scope.aboutbusiness+'&businesskeywords='+$scope.businesskeywords+'&email='+email+'&address1work='+address1work+'&address2work='+address2work+'&address3work='+address3work+'&statenamework='+statenamework+'&mobilenumberwork='+mobilenumberwork+'&communicationaddress='+communication+'&businessgoals='+$scope.businessgoals+'&accomplishments='+$scope.accomplishments+'&interests='+$scope.interests+'&networks='+$scope.networks+'&skills='+$scope.skills+'&idealreferral='+$scope.idealreferral+'&topproduct='+$scope.topproduct+'&topproblemsolved='+$scope.topproblemsolved+'&valuex='+valuex+'&valuey='+valuey+'&valuew='+valuew+'&valueh='+valueh;			
			console.log(link);
			var formData=new FormData();
			formData.append("companylogo",imgInp.files[0]);
			$http({method: 'POST',
				...
				if($scope.getmemberresidentiallandline.length == 0 && $scope.getmemberworklandline.length == 0) {
					...
					if(temp == 2) {
						/*$window.location.href = url + "my_family_detail";*/
						$window.location.href = baseUrl + "my_contact_detail1";
					} else {
						/*$window.location.href = adminurl + "manage_family_detail?memberid="+memberid;*/
						$window.location.href = baseUrl + "edit_profile";
					}					
				}
				/*angular.forEach($scope.getmemberresidentiallandline, function(item) {
					if(item.landlinePhoneNumber) {
