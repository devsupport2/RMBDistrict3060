...
/*	
	...
	//---------------------------  Meeting 
		...
		$scope.setflag2=function(){
			$scope.metmemberidError=0;
			$scope.meetingdateError=0;
			$scope.locationError=0;
			$scope.topicError=0;
		}
		$scope.saveOnetoOne = function(invitedbymemberid) {		
			...
			if(!$scope.metmemberid){
				$scope.metmemberidError=1;
				$scope.metmemberidMsg="Please select member!";
				...
				return;
			}*/ else if(!$scope.meetingdate){
				$scope.meetingdateError=1;
				$scope.meetingdateMsg="Please select meeting date!";
				document.getElementById("meetingdate").focus();
				return;
			} else if(!$scope.location){
				$scope.locationError=1;
				$scope.locationMsg="Please enter meeting location!";
				document.getElementById("location").focus();
				return;
			} else if(!$scope.topic){
				$scope.topicError=1;
				$scope.topicMsg="Please enter topics of conversation!";
...
$scope.editmemberdetail = function(memberid, temp) {
		...
		if(!membercategoryname) {
			$scope.membercategorynameError=1;
			$scope.membercategorynameMsg="Please select type of member";
			document.getElementById("membercategoryname").focus();
			return;
		} else if(!$scope.businesscategoryid) {
			$scope.businesscategoryidError=1;
			$scope.membercategorynameMsg="Please select business category";
			...
			$scope.lastnameError=1;
			$scope.lastnameMsg="Please enter last name";
			document.getElementById("lastname").focus();
			return;
		}else if(!email) {
			$scope.emailError=1;
			$scope.emailMsg("Please enter email id");
			...
			$http({method: 'POST',
				...
				if($scope.getmembereducationdetail.length == 0)	{
					$scope.spin = 0;
					$scope.nospin = 1;
					toastr.success("Member Detail Updated Successfully");
					if(temp == 2) {
						$window.location.href = url+"manage_my_profile11";
					} else {
					...
					if(item.degreeName != null)	{
						...
						$http.post(link).success(function(data, status, headers, config) {
							...
							if($scope.getmembereducationdetail.length == a) {
								$scope.spin = 0;
								$scope.nospin = 1;
								toastr.success("Member Detail Edited Successfully");
								if(temp == 2) {
									$window.location.href = url+"manage_my_profile11";
								} else {
									$window.location.href = url+"edit_profile";
	...
	$scope.setflag=function(){
		...
		$scope.middlenameError=0;
		$scope.lastnameError=0;
		$scope.mobileError=0;
		$scope.emailError=0;
		$scope.businesscategoryidError=0;
