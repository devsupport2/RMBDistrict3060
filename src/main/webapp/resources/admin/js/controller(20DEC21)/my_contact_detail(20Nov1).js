app.controller('manageContactDetailCtrl',function($scope, $http, $window, $filter, $location) {
	...
	$scope.UrlMapper = function(strToMapp)
	{
		var pro = pro1 = pro2 = "";
		pro = strToMapp.replace("&","$");
		pro1 = pro.replace("#","~");
		pro2 = pro1.replace("%","!");
	...
	$scope.editcontactdetail = function(memberid, membercategoryid, tenureplan, temp) {
		...
		/*	$window.alert("Please select address for communication");
			...
			$scope.aboutbusiness = $scope.UrlMapper($scope.aboutbusiness);

			$scope.businesskeywords = $scope.UrlMapper($scope.businesskeywords);

			$scope.businessgoals = $scope.UrlMapper($scope.businessgoals);

			$scope.accomplishments = $scope.UrlMapper($scope.accomplishments);

			$scope.interests = $scope.UrlMapper($scope.interests);

			$scope.networks = $scope.UrlMapper($scope.networks);

			$scope.skills = $scope.UrlMapper($scope.skills);

			$scope.idealreferral = $scope.UrlMapper($scope.idealreferral);

			$scope.topproduct = $scope.UrlMapper($scope.topproduct);
			
			$scope.topproblemsolved = $scope.UrlMapper($scope.topproblemsolved);
