							...
							$http.post(link)
									.success(
											function(data, status, headers,
													config) {
												if (data == "") {
													$window.alert("User Name or Password Incorrect...Try Again bb");
												} else {
			...
			app.controller('eventCtrl',['$scope','$filter','$http','$window','$location',function($scope, $filter, $http, $window, $location) 
			{
				...
				$scope.removeUrlRow = function(eventurltitle) {
					...
					$scope.eventurllist.splice(index, 1);
							...
							$scope.geteventdetails = function(eventid) {
								for (i in $scope.events) {
									if ($scope.events[i].eventId == eventid) {
										...
										$scope.fellowship_id = $scope.events[i].fellowship_id;
							...
							$scope.amount = 0.000;
							$scope.registrationlist=[{'registrationFor' : "RMB District Member",'currencyId' : $scope.currencytype,'currencyTitle' : $scope.currencyTitle,'amount' : $scope.amount},
							...
							$scope.eventrelatedimages2 = [];
								...
								if ($scope.paidedit == undefined || $scope.paidedit == "") {
									...
									var link = baseUrl + 'editEvent?fellowship_id='+fellowship_id+'&eventid='
											+ eventid + '&eventtype='
		...
		$http.get(link).success(
			...
			error(function(data, status, headers, config)
			{
				...
				{					
					...
					{
						//alert(setter);
						$scope.albumimage.push({'imageTitle':$scope.imagetitle, 'imageDescription':imagedescription, 'setter':setter});
									...
									angular.forEach($scope.albumimage,
										function(item)
										{
											if(item.imageTitle != null)
											{
												...
												$scope.set1.push(item.setter);
											}
										});														
														
										var link = baseUrl + 'addAlbumImage?imagetitle='+$scope.titleimage+'&imagedescription='+$scope.desc+'&set1='+$scope.set1;									
								//	alert(link);
										$http({
												method: 'POST',
												url: link,
				...
				$scope.coverimage = function(indexid){
					
					for(var i=0;i<$scope.albumimage.length;i++){
						if(i == indexid){
						//	alert("if");
							$scope.albumimage[i].setter='true';
						}
						else{
//	alert("else");
							$scope.albumimage[i].setter='false';
						}
						console.log($scope.albumimage);
					}
					
				}
				$scope.coverimage1 = function(indexid){
					console.log($scope.albumimagenew);
					for(var i=0;i<$scope.albumimagenew.length;i++){
						if(i == indexid){
						//alert("if"+ indexid);
							$scope.albumimagenew[i].setter='true';
						}
						else{
					//alert("else"+ indexid);
							$scope.albumimagenew[i].setter='false';
						}
					}
					console.log($scope.albumimagenew);
				}
			...
			$scope.getalbum = function(albumid)
			{
				...
				//	alert(link);
					$http.get(link).success(
							function(data, status, headers, config)
							{
								...
								$scope.albumimagenew = $scope.albumimage;
								for(var i=0 ;i<$scope.albumimage.length;i++)
									{
										if($scope.albumimage[i].set1){
											$scope.setter = i;
										}
									}
							}).
				...
				$scope.addAlbumImageRowEdit = function()
				{
					...
					{
						//alert("mk");
						$scope.albumimagenew.push({'imageTitle':$scope.imagetitle, 'description':$scope.imagedescription, 'setter':$scope.setter});
						formData.append("image",image.files[0]);
					}
				};
						...
						angular.forEach($scope.albumimagenew,
							function(item)
							{
								if(item.imageTitle != null)
								{
									...
									$scope.desc.push(item.description);
									$scope.set1.push(item.setter);
								}
							});
				...
				//	alert(link);	
						$http.put(link).success(
								function(data, status, headers, config)
								{
									...
									var link = baseUrl+'editAlbumImage?albumid='+albumid+'&imagetitle='+$scope.title+'&imagedescription='+$scope.desc+'&set1='+$scope.set1;
								//		alert(link);
									$http({
											method: 'POST',
											url: link,
										...
										{   
										  	...
										  	angular.forEach($scope.albumimage,
											   		function(item)
											   		{
														if(item.imageTitle != null)
														{
															var link = baseUrl+'editAlbumImageNew?albumid='+albumid+'&imagetitle='+item.imageTitle+'&imagedescription='+item.description+'&image='+item.image+'&set1='+item.set1;
										    		//	alert(link);
															$http.post(link).success(
									    						function(data, status, headers, config)
									    						{						    							
	...
	app.controller('albumCtrl', ['$scope', '$filter', '$http', '$window', '$location' ,function ($scope, $filter, $http, $window, $location){
		...
		$http.get(link).success(
			...
			error(function(data, status, headers, config)
			{
				...
				//from video link
				$scope.getLink = function(vid_ID){
					$scope.vidLink="//www.youtube.com/embed/"+vid_ID;
					$scope.vidLinkkk="<iframe style='height:auto;' width='220' src='https://www.youtube.com/embed/"+vid_ID+"' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>";
					document.getElementById(vid_ID).innerHTML = $scope.vidLinkkk;
				}
			...
			$scope.getalbum = function(albumid)
			{
				...
				$scope.addAlbumImageRowEdit = function()
				{
					...
					{
						/*$scope.albumimagenew.push({'imageTitle':$scope.imagetitle, 'imageDescription':$scope.imagedescription,'videoUrl':$scope.videoId});
						formData.append("image",image.files[0]);*/
						
						var link = baseUrl+'editAlbumImage?albumid='+$scope.albumid+'&imagetitle='+$scope.imagetitle+'&imagedescription='+$scope.imagedescription+'&videoId='+$scope.videoId;
					    // alert(link);
				...
				$scope.editalbum = function(albumid)
				{
					...
					else
					{
						...
						$http.put(link).success(
								...
								/*	var link = baseUrl+'editAlbumImage?albumid='+albumid+'&imagetitle='+$scope.title+'&imagedescription='+$scope.desc + '&videoId='+$scope.VideoUrl;
...
/*	}*/
		$scope.getDateForMember = function(){
			...
			$scope.fromdate = "0"+1+"/"+"07"+"/"+$scope.currentYear;		
			$scope.joiningdate= $scope.fromdate;
			 
		}
		
		$scope.getMembersByFellowship = function(fellowship_id){

			var link = baseUrl+'getAllMemberNameList?fellowship_id='+fellowship_id;  
            alert(link);
			$http.get(link).success(function(data, status, headers, config) {
				$scope.getAllMemberNamedata = data;
		...
		{			
			if($scope.pageSize == "All") {
				...
				var link = baseUrl+'getMemberByPage?pagesize=50000'+'&startindex=0';
				$http.get(link).success(function(data, status, headers, config) {
...
//typemem = "RMBF";
	...
	app.controller('committeeCtrl', function($scope, $http, $window, $filter, $location) 
			{
						...
						$http.get(link).success(function(data,status,headers,config) {
							...
							$scope.getCommitieByFellowship = function(fellowship_id,role_id)
							{
								...
								var link = baseUrl + 'MembersCommitteeBYFellowship?fellowship_id='+fellowship_id;
								//alert(link);
								$http.get(link).success(function(data,status,headers,config) {
									$scope.memberscommittee = data;
								}).error(function(data,status,headers,config) {
	...
	app.controller('referralCtrl',function($scope, $http, $window, $filter, $location) {
		...
		$scope.saveReferral = function(temp) {
			...
			if($scope.tomemberid == undefined || $scope.tomemberid == ""){
				...
				$scope.spin = 1;				
				var link = baseUrl+'saveReferral1?tomemberid='+$scope.tomemberid+'&referralname='+$scope.referralname+'&referdate='+$scope.referdate+'&referraltype='+$scope.referraltype+'&card='+$scope.card+'&call='+$scope.call+'&referralemail='+$scope.referralemail+'&referralcontactno='+$scope.referralcontactno+'&referraladdress='+$scope.referraladdress+'&comment='+$scope.comment;				
			//	alert(link);
				$http.post(link).success(function(data, status, headers, config) {
	...
	app.controller('chapterSummaryCtrl1',function($scope, $http, $window, $filter, $location) {
		var baseUrl = $location.protocol()+"://"+location.host+u;	
	...
	app.controller('chapterSummaryCtrl',function($scope, $http, $window, $filter, $location) {
		...
		const fellowship_id = urlParams.get('fellowship_id');
		...
		$scope.getChapterSummary = function() {		
			...
			var link = baseUrl+'getChapterSummaryByDate1?fromdate='+$scope.fromdate+'&todate='+$scope.todate+'&fellowship_id='+fellowship_id;
		//	alert("-1-"+link);
			$http.get(link).success(function(data, status, headers, config) {
				$scope.getChapterSummaryByDate = data;	
	...
	app.controller('businessCtrl',['$scope','$filter','$http','$window','$location',function($scope, $filter, $http, $window, $location)
		{
			...
			$scope.getBusinessByFellowship = function (fellowship_id){	
			var link = baseUrl + 'business';			
			$http.get(link).success(function(data, status, headers, config) { 
				$scope.business1 = data;
			}).error(function(data, status, headers, config) {
				$scope.business1 = "Response Fail";
			});		
			}	
			$scope.getBusiness = function (fellowship_id){			
			var link = baseUrl + 'businessByFellowship?fellowship_id='+fellowship_id;				
			$http.get(link).success(function(data, status, headers, config) { 
				$scope.business = data;
			}).error(function(data, status, headers, config) {
			...
			$scope.UpdateBusinessByAdmin = function(thankyouslipid) {
				 
				$scope.getBusinessbyid.thankyouslipid=thankyouslipid;
				 
				var link = baseUrl+'editBusinessByAdmin';
				 //	alert(link);
				 	$http({url: link, method: "POST", data: $scope.getBusinessbyid}).success(function(data, status, headers, config) {
				 	
				 		$scope.getBusinessbydata = data;
				 	
				 		location.reload(true);	
				 		
				 		}).error(function(data, status, headers, config) {
				 			$scope.getBusinessbydata = "Response Fail";
				 		});
			...
			$scope.saveThankYouSlip1 = function(temp) {	
				...
				if($scope.tomemberid == undefined || $scope.tomemberid == ""){
					...
					$scope.spin = 1;				
					var link = baseUrl+'saveThankYouSlip2?tomemberid='+$scope.tomemberid+'&amount='+$scope.amount+'&slipdate='+$scope.slipdate+'&businesstype='+$scope.businesstype+'&referraltype='+$scope.referraltype+'&comment='+$scope.comment+'&Frommemberid='+$scope.metmemberid;				
					//alert(link);
					$http.post(link).success(function(data, status, headers, config) {
						$scope.savethankyouslip = data;
						$scope.spin = 0;					
	...
	app.controller('meetingCtrl',['$scope','$filter','$http','$window','$location',function($scope, $filter, $http, $window, $location)
		{			
			...
			$scope.getMeetingSuperAdmin = function(fellowship_id){		
			var link = baseUrl + 'meeting';			
			$http.get(link).success(function(data, status, headers, config) { 
				$scope.meeting1 = data;
			}).error(function(data, status, headers, config) {
				$scope.meeting1 = "Response Fail";
			});		
			}			
			$scope.getMeeting = function(fellowship_id){			
				var link = baseUrl + 'meetingByFellowship?fellowship_id='+fellowship_id;				
			//	alert(link);	
				$http.get(link).success(function(data, status, headers, config) { 
					$scope.meeting = data;
				}).error(function(data, status, headers, config) {
			...
			$scope.UpdateMeetingByAdmin = function(oneToOneId) {
 
				$scope.getMeetingbyid.oneToOneId=oneToOneId;
				 
				var link = baseUrl+'editMeetingByAdmin';
				 	//alert(link);
				 	$http({url: link, method: "POST", data: $scope.getMeetingbyid}).success(function(data, status, headers, config) {
				 	
				 		$scope.getMeetingbyiddata = data;
				 	
				 		location.reload(true);	
				 	//	$('#myModal').modal('hide');
				 		
				 		}).error(function(data, status, headers, config) {
				 			$scope.getMeetingbyiddata = "Response Fail";
				 		});
			...
			$scope.saveOnetoOne1 = function(invitedbymemberid) {			
				...
				if($scope.metmemberid == undefined || $scope.metmemberid == ""){
					...
					$scope.spin = 1;				
					var link = baseUrl+'saveOnetoOne1?metmemberid='+$scope.metmemberid+'&invitedbymemberid='+invitedbymemberid+'&meetingdate='+$scope.meetingdate+'&location='+$scope.location+'&topic='+$scope.topic;				
			//	alert(link);
					$http.post(link).success(function(data, status, headers, config) {
						$scope.saveonetoone = data;
						$scope.spin = 0;					
	...
	app.controller('referenceCtrl',['$scope','$filter','$http','$window','$location',function($scope, $filter, $http, $window, $location)
		{			
			...
			var link = baseUrl + 'reference1';		
			$http.get(link).success(function(data, status, headers, config) { 
				$scope.reference = data;
			}).error(function(data, status, headers, config) {
				$scope.reference = "Response Fail";
			});	
			$scope.getReffer = function (fellowship_id){			
			var link = baseUrl + 'reference?fellowship_id='+fellowship_id;				
			$http.get(link).success(function(data, status, headers, config) { 
				$scope.reference = data;
			}).error(function(data, status, headers, config) {
				$scope.reference = "Response Fail";
			...
			$scope.saveReferral = function(temp) {
				...
				if($scope.tomemberid == undefined || $scope.tomemberid == ""){
					...
					$scope.spin = 1;				
					var link = baseUrl+'saveReferral1?tomemberid='+$scope.tomemberid+'&referralname='+$scope.referralname+'&referdate='+$scope.referdate+'&referraltype='+$scope.referraltype+'&card='+$scope.card+'&call='+$scope.call+'&referralemail='+$scope.referralemail+'&referralcontactno='+$scope.referralcontactno+'&referraladdress='+$scope.referraladdress+'&comment='+$scope.comment+'&closedate='+$scope.closedate+'&referalstatus='+$scope.referalstatus+'&apprvalue='+$scope.apprvalue+'&Frommemberid='+$scope.Frommemberid;				
				//alert(link);
					$http.post(link).success(function(data, status, headers, config) {
						$scope.savereferral = data;
						$scope.spin = 0;
