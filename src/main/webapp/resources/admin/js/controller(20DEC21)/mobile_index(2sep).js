app.controller('MobileIndexCtrl',function($scope, $http, $window, $filter, $location) {
...
//alert(link);
	...
	$scope.getTopMemberMeeting = function() {	
		...
		var fromdate = $filter('date')(new Date(new Date().setDate(new Date().getDate() - 30)),'dd/MM/yyyy'); 
	...
	//	alert(fromdate);
		...
		//alert(link);
		$http.get(link).success(function(data, status, headers, config) {
			$scope.getTopmemberMeetingdata = data;			
		}).error(function(data, status, headers, config) {
			$scope.getTopmemberMeetingdata = "Response Fail";
		});
	}
	...
	$scope.getTopMemberBusiness = function() {	
		...
		$scope.fromdateTop = $filter('date')(new Date($scope.y, $scope.m - 1, 1),'dd/MM/yyyy'); 
		$scope.todateTop = $filter('date')(new Date($scope.y, $scope.m  , 0),'dd/MM/yyyy'); 
		...
		var link = baseUrl+'getTopMemberBusiness?fromdate='+fromdate+'&todate='+todate;			
		//alert(link);
		$http.get(link).success(function(data, status, headers, config) {
...
//-------------------Count Refferal--------------------------
		...
		//	$scope.startdate3= document.getElementById("summaryfrom").value;
			//$scope.enddate3= document.getElementById("summaryto").value;
				...
				var link = baseUrl+'getThirdSummary?fromdate='+fromdate+'&todate='+todate;
				//alert(link);
				$http.get(link).success(function(data, status, headers, config) {
