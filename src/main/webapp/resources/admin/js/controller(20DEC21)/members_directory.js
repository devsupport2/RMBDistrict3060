...
/*	var link = baseUrl+'getMemberForMembersDirectoryByPage?pagesize='+$scope.pageSize+'&startindex='+$scope.currentPage;
	...
	*/
				var link = baseUrl+'getAllFellowshipName';  
						//alert(link);
						$http.get(link).success(function(data, status, headers, config) {
							$scope.fellowshipnamedata = data;
						
					$scope.passValue( );
						//	alert(typeof $scope.fellowshipnamedata[0].fellowship_id);
						}).error(function(data, status, headers, config) {
							$scope.fellowshipnamedata = "Response Fail";
						});
				$scope.getFellowshipById = function(){
							const params = new URLSearchParams(window.location.search);
								if(params.has('fellowship_id')){
										
											$scope.fellowship_id=(params.get('fellowship_id'));
										$scope.fellowship_id=parseInt( $scope.fellowship_id);
				...
				$scope.passValue = function(){
					$scope.fellowship_id=$scope.fellowship_id;
				//	alert($scope.fellowship_id);
				}			
...
/*	
	...
	$http.get(link).success(function(data, status, headers, config) {
		$scope.MembersCommitteedata = data;				
	}).error(function(data, status, headers, config) {
		$scope.MembersCommitteedata = "Response Fail";
	});	*/
