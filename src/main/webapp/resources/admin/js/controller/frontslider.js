...
// ------------------------ Top Meeting Count
	...
	//--------------------------------------------- Album
	   $scope.currentPage = 0;
		...
		//alert($scope.Id);
		//alert(membertype);
				if(membertype== " RMBFB Member" || membertype==" RMBF" || membertype==" R I Representative"){
	...
	$scope.GetDetailsOfcharges=function(eventId){
		...
		//alert("hello");
		console.log($scope.EventsByImagesdata);
		
	const EventData =$scope.EventsByImagesdata.find(element => element.eventId);
