...
<html lang="en">
	...
	<body ng-app="rcbs" ng-controller="referenceCtrl"  ng-init="getReffer(<%=session.getAttribute("fellowshipId") %>)">
			...
			<div class="container">
				<div class="row panel panel-primary" style="width:auto;background-color:#eee;">
					...
					<div class="col-md-3" >
					 ...
					 <div class="col-md-3" ng-show="<%=session.getAttribute("roleId") %> == '2'" style="margin-top:15px">
					
					</div>		
					 <div class="col-md-3" ng-show="<%=session.getAttribute("roleId") %> == '1'" style="margin-top:15px">
					
						<select ng-model="fellowship_id" id="fellowship_id" name="fellowship_name" ng-change="getReffer(fellowship_id)" class="form-control input-lg2">
						...
						<form role="form" ng-submit="deleteRef()">
							<div class="table-responsive">
								<table id="mytable" class="table table-bordred table-striped">
									...
									<tbody>
									<!-- | filter:search | startFrom:currentPage*pageSize | limitTo:pageSize"   ng-if="item.role_id != 1"-->
										<tr ng-repeat="item in reference  | filter: { fellowship_id: fellowship_id}"  ng-if="item.role_id != 1" style="cursor:pointer;">											
											
											<td data-toggle="modal" data-target="#myModal" ng-click='getReferenceDetail(item.memberReferralId ,item.fromFirstName, item.fromLastName)'>{{item.fromFirstName}} {{item.fromLastName}} <br> {{item.fellowship_name}}</td>
											<td data-toggle="modal" data-target="#myModal" ng-click='getReferenceDetail(item.memberReferralId,item.fromFirstName, item.fromLastName)'>{{item.memberFirstName}} {{item.memberLastName}}<br> {{item.fellowship_name}}</td>
						...
						<!-- 		<h4 class="modal-title">Reference from {{fromMemberName}}</h4> -->
							...
							<div class="modal-body">
								<form>
									<div class="row">
									<div class="col-md-4">														
											<lable>From<font color="red" size="3">*</font></lable>
											<select class="form-control" name="Frommemberid" id="Frommemberid" ng-model="Frommemberid">
												<option value="">Select</option>
												<option ng-repeat="item in getmember "  value="{{item.memberId}}">{{item.memberFirstName}} {{item.memberMiddleName}} {{item.memberLastName}}</option>
											</select>														
										</div>
							...
							<div class="row">
									...
									<div class="form-group">
											<lable>Date<font color="red" size="3">*</font></lable>
											<input type="text"  class="form-control" id="refferal" ng-model="referdate" title="Refer Date" placeholder="DD/MM/YYYY" style="width: 100%;"/>														
									</div>
										</div>
										<div class="col-md-4">
										<div class="form-group">
											<lable>Close by Date<font color="red" size="3">*</font></lable>
											<input  type="text"  class="form-control" id="closebydate" ng-model="closebydate" title="Close by Date" placeholder="DD/MM/YYYY" style="width: 100%;"/>														
										</div>
										</div>
										</div>
									...
									<div class="row">
										<div class="col-md-4">
											<div class="form-group">
												<lable>Referral Type<font color="red" size="3">*</font></lable><br>
											<select class="form-control" style="width: 100%;" name="referraltype" id="referraltype" ng-model="referraltype" > 
									...
									<div class="row">
										<div class="col-md-12 text-center">
											<button class="btn btn-primary" ng-click="saveReferral(flag='save')" style="background-color:  #005daa;"><i class="fa fa-refresh fa-spin" ng-show="spin == 1"></i> Add </button>
										</div>													
									</div>												
								</form>																							
							</div>						
