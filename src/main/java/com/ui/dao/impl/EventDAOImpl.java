...
import com.ui.model.EventPaymentSuccess;
...
//import com.ui.model.MouldCategory;
...
public class EventDAOImpl implements EventDAO {
	...
	//String sql = "select event_id, event_type_id, event_title, event_subtitle, event_venue, event_map_location, event_date, event_time, event_description, registration, paid, rmbfb_member, rotarian, others, status, created_by, created_date, created_time, ip_address from event where status=?";
	    String sql="select ei.event_image_title,ei.event_image_id,ei.image,ff.fellowship_name as fellowship_name,ee.fellowship_id as fellowship_id,ee.event_id,event_type_id, event_title, event_subtitle, event_venue, event_map_location, event_date,event_time, event_description, registration, paid, rmbfb_member, rotarian, others, ee.status,ee.created_by,ee.created_date, ee.created_time, ee.ip_address from event ee left join fellowship ff on ff.fellowship_id=ee.fellowship_id left join event_image ei on ei.event_image_id in (SELECT MIN(event_image_id) FROM event_image where event_id=ee.event_id group by event_id ) where ee.status=? and event_date < curdate() order by ee.event_id desc";
	    //select ei.event_image_title,ei.event_image_id,ei.image,ff.fellowship_name as fellowship_name,ee.fellowship_id as fellowship_id,event_id, event_type_id, event_title, event_subtitle, event_venue, event_map_location, event_date, event_time, event_description, registration, paid, rmbfb_member, rotarian, others, ee.status, ee.created_by,ee.created_date, ee.created_time, ee.ip_address from event ee left join fellowship ff on ff.fellowship_id=ee.fellowship_id left join event_image ei on ei.event_image_id in (SELECT MIN(event_image_id) FROM event_image where event_id=ee.event_id group by event_id ) where ee.status=? and event_date >= curdate() order by ee.event_id desc"
	    Connection conn = null;
	    try {
	        conn = dataSource.getConnection();
	        PreparedStatement ps = conn.prepareStatement(sql);
	        ...
	        while (rs.next()) {
	            event = new Event(rs.getInt("event_id"), rs.getInt("event_type_id"), rs.getString("event_title"),
	                    ...
	                    rs.getString("created_time"), rs.getString("ip_address"),rs.getString("fellowship_name"),rs.getInt("fellowship_id"));
		...
		try {
			...
			while (rs.next()) {
				event = new Event(rs.getInt("event_id"), rs.getInt("event_type_id"), rs.getString("event_title"),
						...
						rs.getFloat("rmbfb_member"), rs.getFloat("rotarian"), rs.getFloat("others"),
						rs.getString("status"), rs.getInt("created_by"), rs.getString("created_date"),
						rs.getString("created_time"), rs.getString("ip_address"),rs.getString("fellowship_name"),rs.getInt("fellowship_id"),rs.getString("event_image_title"),rs.getString("image"));
	...
	public List<Event> getAllEventsByImages() {
		logger.info("********** Inside Get All Events **********");
		List<Event> Event = new ArrayList<Event>();
		String s = "y";
...
String sql = null;
	...
	public void addEvent(Event e) {
		...
		String sql = "insert into event (event_type_id,event_title,event_subtitle,event_venue,event_map_location,event_date,event_time,event_description, registration, paid, rmbfb_member, rotarian, others, status,created_by,created_date,created_time,ip_address,fellowship_id) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		try {
			...
			ps.setInt(19, e.getFellowship_id());
	...
	public void editEvent(Event e) {
		...
		String sql = "update event set event_type_id=?, event_title=?, event_subtitle=?, event_venue=?, event_map_location=?, event_date=?, event_time=?, event_description=?, registration=?, paid=?, rmbfb_member=?, rotarian=?, others=?, created_by=?, created_date=?, created_time=?, ip_address=?,fellowship_id=?  where event_id=?";
		...
		try {
			...
			ps.setInt(18, e.getFellowship_id());
			ps.setInt(19, e.getEventId());
	...
	public Event getEventDetailById(int id) {
		...
		// String s = "y";
		String sql = "select event_id, event_type_id, event_title, event_subtitle, event_venue, event_map_location, event_date, event_time, event_description, registration, paid,fellowship_id from event where event_id=?";
		Connection conn = null;
		try {
			conn = dataSource.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			...
			while (rs.next()) {
				event = new Event(rs.getInt("event_id"), rs.getInt("event_type_id"), rs.getString("event_title"),
						rs.getString("event_subtitle"), rs.getString("event_venue"), rs.getString("event_map_location"),
						rs.getString("event_date"), rs.getString("event_time"), rs.getString("event_description"), rs.getString("registration"), rs.getString("paid"),rs.getInt("fellowship_id"));
			}
			rs.close();
			ps.close();
...
logger.info("********** INSIDE GET MEMBER EVENT REGISTRATION STATUS **********");
	...
	@Override
	public EventCharges getEventChargesById(int eventChargesId) {
		logger.info("********** INSIDE GET EVENT DETAIL BY ID IMPL **********");
		// String s = "y";
		String sql = "select event_charges_id,event_id,registration_for,currency_id,amount from event_charges where event_charges_id=?";
		Connection conn = null;
		try {
			conn = dataSource.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);

			ps.setInt(1, eventChargesId);

			EventCharges event = null;
			ResultSet rs = ps.executeQuery();

			while (rs.next()) {
				event = new EventCharges(rs.getInt("event_charges_id"), rs.getInt("event_id"), rs.getString("registration_for"),
						rs.getInt("currency_id"), rs.getFloat("amount"));
			}
			rs.close();
			ps.close();
	...
	@Override
	public EventRegistration getLastEventDetails() {
	       logger.info("+++++ GET LAST ENQUIRY ID IMPL +++++");
	       String sql = "select event_registration_id,event_id,first_name,last_name,last_name,email_id,mobile_no from event_registration order by event_registration_id desc limit 0,1";
	...
	public int getLastEventRegistrationId() {
		String sql = "select event_registration_id from event_registration order by event_registration_id desc limit 0,1";
		int id = 0;
		Connection conn = null;
		logger.info("********** Inside get last Event Id Impl **********");
		try {
			...
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				id = rs.getInt("event_registration_id");
			}
			rs.close();
			ps.close();
			return id;
	...
	@Override
	public void addPaymentStatusForSucces(EventPaymentSuccess eventPaymentSuccess) {


	    logger.info("+++++ ADD ENQUIRY ASSIGN +++++");
	    String sql = "insert into EventPaymentStatus(event_registration_id, ipAddress) values (?,?)";

	    Connection conn = null;
	    try {
	        conn = dataSource.getConnection();
	        PreparedStatement ps = conn.prepareStatement(sql);
	        ps.setInt(1, eventPaymentSuccess.getEvent_registration_id());
	       //ps.setInt(2, eventPaymentSuccess.getCreatedBy());
	        ps.setString(2, eventPaymentSuccess.getIpAddress());
	        ps.executeUpdate();
	...
	@Override
	public void addPaymentStatusForFail(EventPaymentSuccess eventPaymentSuccess) {
		



	    logger.info("+++++ ADD ENQUIRY ASSIGN +++++");
	    String sql = "insert into EventPaymentStatus(event_registration_id, PaymentStatus, ipAddress) values (?,?,?)";
	    ...
	    try {
	        ...
	        PreparedStatement ps = conn.prepareStatement(sql);
	        ps.setInt(1, eventPaymentSuccess.getEvent_registration_id());
	       ps.setString(2, eventPaymentSuccess.getPaymentStatus());
	        ps.setString(3, eventPaymentSuccess.getIpAddress());
	        ps.executeUpdate();
	    } catch (SQLException ex) {
	        throw new RuntimeException(ex);
	    } finally {
	...
	@Override
	public   List<EventRegistration> GetAllEventEmailOrMobile() {

	    logger.info("+++++ GET ALL MOULD CATEGORY +++++");
	    List<EventRegistration> sta = new ArrayList<EventRegistration>();
	    //String s = "y";

	    String sql = "select email_id,mobile_no from event_registration";
	    Connection conn = null;
	    try {
	        conn = dataSource.getConnection();
	        PreparedStatement ps = conn.prepareStatement(sql);
	        //ps.setString(1, s);
	        EventRegistration cat = null;
	        ResultSet rs = ps.executeQuery();
	        while (rs.next()) {
	            cat = new EventRegistration();
	            cat.setEmailId(rs.getString("email_id"));
	            cat.setMobileNo(rs.getString("mobile_no"));
