...
public class MembersCommitteeDAOImpl implements MembersCommitteeDAO {
	...
	public List<MembersCommittee> getAllMembersCommittee()
	{
		...
		String s = "y";
		String sql = "select  m.member_email,mc.fellowship_id,ff.fellowship_name as fellowship_name, mc.members_committee_id, mc.sequence, mc.designation, mc.member_id, mc.rotary_year_id, m.member_first_name, m.member_last_name, r.rotary_year_title, m.member_profile_picture, m.member_name_title from members_committee mc, rotary_year r, members m , fellowship ff where m.member_id = mc.member_id and ff.fellowship_id = mc.fellowship_id and r.rotary_year_id = mc.rotary_year_id and mc.status=? order by mc.sequence";
	...
	public List<MembersCommittee> getAllMembersCommitteeByFellowshipId(int fellowship_id)
	{
		logger.info("********** INSIDE GET ALL MEMBERS COMMITTEE BY Fellowship IMPL **********");
		List<MembersCommittee> MembersCommittee = new ArrayList<MembersCommittee>();
		String s = "y";
		String sql = "select  m.member_email,mc.fellowship_id,ff.fellowship_name as fellowship_name, mc.members_committee_id, mc.sequence, mc.designation, mc.member_id,mc.rotary_year_id, m.member_first_name, m.member_last_name, r.rotary_year_title,m.member_profile_picture, m.member_name_title from members_committee mc, rotary_year r,members m,fellowship ff where m.member_id = mc.member_id and ff.fellowship_id = mc.fellowship_id and r.rotary_year_id = mc.rotary_year_id and mc.status=? and mc.fellowship_id=? order by mc.sequence";
		Connection conn = null;
		try
		{
			conn = dataSource.getConnection();
	...
	public List<MembersCommittee> getAllMembersCommitteeByRotaryYear(int rotaryyearid)
	{
		...
		List<MembersCommittee> MembersCommittee = new ArrayList<MembersCommittee>();
		String s = "y";
		String sql = "select ff.fellowship_name,mc.fellowship_id, mc.members_committee_id, mc.sequence, mc.designation, mc.member_id, mc.rotary_year_id, m.member_first_name, m.member_last_name, r.rotary_year_title, m.member_profile_picture, m.member_name_title from members_committee mc, rotary_year r, members m ,fellowship ff where m.member_id = mc.member_id and r.rotary_year_id = mc.rotary_year_id  and ff.fellowship_id = mc.fellowship_id and mc.status=? and mc.rotary_year_id = ? order by mc.sequence";
		Connection conn = null;
		try
		{
			conn = dataSource.getConnection();
